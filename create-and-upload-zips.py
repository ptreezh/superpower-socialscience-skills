...
for expert_dir in expert_dirs:
    ...
    try:
        ...
        if zip_size < 100:
            print(f"  ❌ ZIP 文件太小 ({zip_size} 字节)，跳过上传")
            failed += 1
            continue
        ...
        # 上传 ZIP 文件到服务器
        print(f"  上传到服务器...")
        upload_cmd = f'curl -T "{zip_path}" "ftp://{FTP_HOST}/web/{skill_name}/{skill_name}.zip" --user {FTP_USER}:{FTP_PASS} --ftp-pasv'
        
        # 先创建远程目录
        mkdir_cmd = f'curl -X MKD "ftp://{FTP_HOST}/web/{skill_name}" --user {FTP_USER}:{FTP_PASS} --ftp-pasv'
